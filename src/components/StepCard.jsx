    ...
    return (
        ...
        >
                ...
                <div className={`w-full ${image ? 'md:w-1/2' : ''} flex flex-col justify-center`}>
                    <div className="flex items-center gap-2 md:gap-3 mb-2 md:mb-3">
                        <div className={`p-1.5 md:p-2 rounded-lg 
                            ${isIA || (image && !isMuni) ? 'bg-neon-cyan/10 text-neon-cyan' : ''}
                            ${isMuni ? 'bg-neon-orange/10 text-neon-orange' : ''}
                            ${!isIA && !isMuni && !image ? 'bg-neon-pink/10 text-neon-pink' : ''}
                        `}>
                            <Icon size={16} className={`${isIA || isMuni ? 'animate-pulse' : ''} md:w-5 md:h-5`} />
                        </div>
                        <h3 className={`text-sm md:text-xl font-bold uppercase tracking-tight 
                            ...
                            ${!isIA && !isMuni ? 'bg-gradient-to-r from-white to-white/60 bg-clip-text text-transparent' : ''}
                        `}>
                            {title}
                        </h3>
                    </div>
            ...
            <div className={`absolute -bottom-4 -right-4 w-16 h-16 blur-2xl rounded-full transition-colors 
                ...
                ${!isIA && !isMuni ? 'bg-white/5 group-hover:bg-neon-cyan/10' : ''}
            `} />
        </motion.div>
    );
};
